...
2. - [ ] **Shell injection via source path in `makeUsb`/`calculateSourceSize`** — `src/mainwindow.cpp:154`, `:156`, `:157`, `:993`
...
3. - [ ] **Configurable `LUM` binary name is broken by the privileged-helper allowlist** — `src/mainwindow.cpp:51-59` vs `src/helper.cpp:56-65`
...
4. - [ ] **Error dialog drops the failing path due to missing placeholder** — `src/mainwindow.cpp:732`
